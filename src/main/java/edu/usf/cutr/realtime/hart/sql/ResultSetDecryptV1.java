...
public class ResultSetDecryptV1 {
	private ResultSet rs;
	private ResultSetMetaData rsmd;
	private HashMap<String, Integer> columnInfo;

	public ResultSetDecryptV1(ResultSet rs){
		this.rs = rs;
		try {
	...
	public ArrayList<TransitDataV1> decrypt(){
		ArrayList<TransitDataV1> transitData = new ArrayList<TransitDataV1>();
		try {
			while(rs.next()){
				TransitDataV1 td = convertRowToTransitData();
			    if(td!=null)
			        transitData.add(td);
			}
		} catch (SQLException e) {
	...
	public TransitDataV1 convertRowToTransitData() {
        Iterator<String> columnNames = columnInfo.keySet().iterator();
        
        TransitDataV1 td = null;
